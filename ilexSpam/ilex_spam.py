...
async def ilexspam(client: Client, message: Message):
    ilex = message.text.split(" ", 2)

    if  len(ilex) == 3:
        quantity, spam_text = int(ilex[1]), ilex[2]
...
async def pspam(client: Client, message: Message):
    ...
    ilexp = message.text.split(" ", 2)
    if len(ilexp) > 1:
        quantity = int(ilexp[1])
        for _ in range(quantity):
            porm = choice(PORM)
